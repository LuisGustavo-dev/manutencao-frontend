...
function EquipamentoPage() {
  ...
  const [status, setStatus] = useState<"Disponível" | "Manutencao" | null>(
    null
  );
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:3340/equipamento/${id}`,
          {
            method: "GET",
          }
        );
  ...
  useEffect(() => {
    ...
    const fetchChecklist = async () => {
      try {
        const response = await fetch(
          `http://localhost:3340/equipamento/${id}/realtime`,
          {
            method: "GET",
            headers: {
    ...
    return (
      <main className="flex min-h-screen flex-col items-center justify-center">
        ...
        <p className="text-muted-foreground">
          {error || "O ID solicitado não existe."}
        </p>
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        {status === "Disponível"
                          ...
                          : "Este equipamento tem um chamado ativo."}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {role === "Cliente" && (
                  <>
                    {status === "Disponível" && (
                      ...
                      >
                        <AlertCircle className="mr-2 h-5 w-5" /> Solicitar
                          ...
                          <div className="flex-1 space-y-1">
                            <div className="flex justify-between items-start">
                              ...
                              {item.estado && (
                                <Badge
                                  variant="secondary"
                                  className="text-[10px] h-5 bg-green-200 text-green-800 hover:bg-green-200 dark:bg-green-900 dark:text-green-300"
                                >
                                  Concluído
                                </Badge>
                              )}
