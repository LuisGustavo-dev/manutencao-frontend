...
export function FormChecklistManutentorCorretiva({ 
  ...
  const [itemStates, setItemStates] = useState<AllItemStates>(() => {
    return Object.fromEntries(
      allItems.map(item => [item.id, { status: 'pending', laudo: '', files: [] }])
    );
  });

  // --- 6. Lógica de Validação e Conclusão ---
  
  const updateItemStateInParent = (id: string, newState: ChecklistItemState) => {
    setItemStates(prev => ({
      ...prev,
      [id]: newState
    }));
  ...
  const isItemCompleted = (item: ChecklistItemState) => {
    ...
    if (item.status === 'erro' && item.laudo.trim() !== "" && item.files.length > 0) return true;
    return false;
  };

  const { isPainelEletricoCompleted, isCompressorCompleted, isResumoCompleted } = useMemo(() => {
    const isTabCompleted = (itemIds: string[]) => {
      return itemIds.every(id => isItemCompleted(itemStates[id]));
    };
    
    return {
      isPainelEletricoCompleted: isTabCompleted(painelEletricoItems.map(i => i.id)),
  ...
  return (
    <>
      <DialogHeader> 
        ...
        <DialogDescription>
          Preencha o checklist para dar baixa nesta OS. Cada item é salvo individualmente.
        </DialogDescription>
      </DialogHeader>
          ...
          <div className="space-y-6 py-4 max-h-[50vh] overflow-y-auto pr-2">
            {painelEletricoItems.map(item => (
              <ChecklistItem
                ...
                token={token}
                onStateChange={(newState) => updateItemStateInParent(item.id, newState)}
              />
            ))}
          </div>
        </TabsContent>
      ...
      <DialogFooter className="pt-4">
        <Button variant="outline" type="button" onClick={onClose} disabled={isFinishing}>
          Cancelar
        </Button>
...
function ChecklistItem({ id, label, equipamentoId, token, onStateChange }: ChecklistItemProps) {
  ...
  useEffect(() => {
    ...
    const checkCompletion = () => {
      ...
      if (status === 'erro' && laudo.trim() !== "" && files.length > 0) return true;
      return false;
    };
  ...
  const handleApiSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3340/chamado/atualizar-por-check/equipamento/${equipamentoId}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  return (
    <div 
      ...
      ${status === 'ok' ? 'bg-green-50 border-green-200 dark:bg-green-900/20' : ''}
      ${status === 'erro' && !isComplete ? 'bg-destructive/5 border-destructive/20' : ''}
      ${status === 'erro' && isComplete ? 'bg-amber-50 border-amber-200 dark:bg-amber-900/20' : ''}
    `}>
      ...
      {status === 'erro' && (
        <div className="mt-4 space-y-4 pt-4 border-t border-destructive/20">
          <div className="space-y-2">
            <Label htmlFor={`${id}-laudo`} className="text-destructive font-semibold flex items-center gap-1">
          ...
          <div className="space-y-2">
            <Label htmlFor={`${id}-files`} className="text-destructive font-semibold flex items-center gap-1">
...
function MiniFileUploader({ files, onFileChange, onRemoveFile, disabled=false }: MiniFileUploaderProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);

  return (
    <div className="space-y-2">
      <div 
        className={`flex items-center justify-center p-3 border-2 border-dashed rounded-lg text-muted-foreground ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:bg-secondary/20'}`}
      ...
      {files.length > 0 && (
        <ul className="space-y-1 max-h-24 overflow-y-auto">
          {files.map((file, index) => (
            <li key={index} className="text-sm flex justify-between items-center p-1.5 bg-slate-50 dark:bg-slate-800 rounded-md">
