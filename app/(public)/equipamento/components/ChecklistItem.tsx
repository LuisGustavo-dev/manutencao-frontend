  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <Label htmlFor={`radio-${id}`} className="font-semibold text-md">
          {label}
        </Label>
      ...
      {status === "erro" && (
        <div className="mt-4 pt-4 border-t border-dashed space-y-3">
          <Label htmlFor={`obs-${id}`} className="text-sm">
            Descreva o problema e a solução aplicada:
          </Label>
          <Textarea
            id={`obs-${id}`}
            placeholder="Ex: Relé de fase queimado, foi substituído."
            className="mt-1"
          />
          <Label htmlFor={`file-${id}`} className="text-sm">
            Anexar evidência (foto/vídeo)
          </Label>
